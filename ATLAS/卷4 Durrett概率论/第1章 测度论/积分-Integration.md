...
第二步把简单函数放宽到「有界函数」，但暂时只处理「在一个有限测度集合 $E$ 外取 $0$」的 $f$。这个「紧支撑」条件保证不会出现 $\infty$ 的麻烦。
...
这里必须证明「上确界等于下确界」，定义才是良定的。记 $\sup_{\phi\leq f}\int\phi=\alpha$、$\inf_{\psi\geq f}\int\psi=\beta$。
...
**直觉**：$f\wedge n$ 是「把 $f$ 超过 $n$ 的部分削平」，$1_{E_n}$ 是「把 $E_n$ 外的部分砍掉」，两者配合就把无界、无紧支撑的 $f$ 变成了一列「有界且在有限区域上非零」的函数。引理说的是：这列函数的积分单调上升，极限恰好就是 $\int f$。（这里正是用到了 $\mu$ 的 $\sigma$-有限性——它保证能找到一列 $E_n\uparrow\Omega$ 且每块测度有限。）
...
>$$
>
>其中 $a\vee b=\max(a,b)$。显然 $f=f^{+}-f^{-}$，$|f|=f^{+}+f^{-}$，且 $f^{+},f^{-}\geq 0$。
>
> ^def-e5f6a7
