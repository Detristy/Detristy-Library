...
这个公式的几何意义十分直观：以 $\overrightarrow{M_1M_0}$ 和 $\mathbf{s}$ 为邻边的平行四边形面积除以底边长 $|\mathbf{s}|$，即为高——也就是点到直线的距离。这直接源于 [[向量及其运算|向量积的几何意义]]。
...
事实上，这个公式的分子是 $\mathbf{s}_1$、$\mathbf{s}_2$、$\overrightarrow{M_1M_2}$ 三向量构成的平行六面体的体积（即 [[向量及其运算|混合积]] 的绝对值），分母是以 $\mathbf{s}_1$、$\mathbf{s}_2$ 为邻边的平行四边形面积。两者相除即得平行六面体在公垂线方向上的"高度"，正是两异面直线的距离。
